...
will slow down the simulation as this is done in the python layer.
...
.. note::
    ...
    While the current C++ Basilisk module development requires module variables to be private
    and have associated setter and getter methods, many prior C++ modules don't follow this rule
    and the user configurable variables are public variables.  If they are public variables, then
    they can be set and read with the same syntax as the variables of a C-module.  See the
    module documentation for information if the module variables are public or private.
...
private, then it is set using the setter method ``someModule.setSomeVariable(...)``.
...
Logging a publically available variable from a module is similar to recording one from a message::
...
If the C++ module variable is private, then it must be accessed by the logger through a getter method.
The logger must be setup differently in this case.  Import the package
``Basilisk.utilities.pythonVariableLogger``.  Next, the logger is setup directly with the associated
getter method using::

    moduleLogger = pythonVariableLogger.PythonVariableLogger({"variableName": lambda _: module.getVariableName()})
